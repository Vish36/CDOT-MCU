...
MCUURL_SIP::MCUURL_SIP(const msg_t *msg, Directions dir)
  : MCUURL()
{
cout << "CDOT-MCU: In MCUURL() function";
...
{
PTRACE(1, trace_section << "vish: In Init() function");
...
PTRACE(1, trace_section << "vish: In MergeSipCaps() function");
  scap = -1; vcap = -1;
...
{
	PTRACE(1, trace_section << "vish: In SendRequest() function");
...
{
	PTRACE(1, trace_section << "vish: In invite_request_cb() function");
...
{
	PTRACE(1, trace_section << "vish: In invite_response_cb() function");
  PWaitAndSignal m(connMutex);
...
BOOL MCUSipEndPoint::SipMakeCall(PString from, PString to, PString & callToken)
{
	...
	PTRACE(1, trace_section << "vish: In SipMakeCall() function");
...
int MCUSipEndPoint::SipReqReply(const msg_t *msg, msg_t *msg_reply, unsigned status, const char *status_phrase)
{
	...
	PTRACE(1, trace_section << "vish: In SipReqReply() function");
...
static int dialout_counter=0;
int MCUSipEndPoint::ProcessSipEvent_cb(nta_agent_t *agent, msg_t *msg, sip_t *sip)
{
        PTRACE(1, trace_section << "vish: In ProcessSipEvent_cb()function");
  ...
  if(request == sip_method_invite)
  {
	...
	//CDOT-MCU changes STARTS
	//extracting reason
	/*
	BACKGROUND:

	Though CDOT-MCU provides dial-out facility.There is a need to provide dial-out facility in MAX-NG conferencing module as per CORE-AT document
	
	INTRODUCTION:***********************************

	To provide dialout facility a proprietary SIP INVITE from the CDOT-NGMS will be received in standard format.
	This INVITE contains REASON field with text "dial-out"
	Please see the INVITE format below:

	INVITE sip:room_name@CDOT-MCU_IP SIP/2.0
	Via: SIP/2.0/UDP CDOT-NGMS_IP:5060;branch=<z9hG4bK561babeb>
	Max-Forwards: 70
	from: <sip:dialout_number@CDOT-NGMS_IP>;tag=as7bea0a9c
	To: <sip:room_name@CDOT-NGMS_IP>
	Contact: <sip:dialout_number@CDOT-NGMS_IP:5060>
	Call-ID: <some_random_string>@CDOt-NGMS_IP:5060
	CSeq: 102 INVITE
	User-Agent:CDOT-NGMS
	Date:<some valid date> 
	Allow: INVITE, ACK, CANCEL, OPTIONS, BYE, REFER, SUBSCRIBE, NOTIFY, INFO, PUBLISH, MESSAGE
	Supported: replaces, timer
	Reason: SIP ;cause=<some cause> ;text="dial-out"
	...
	After carefully examining the received invite,we need to create a dail out session to invite the subscriber(dialout_number) in a room (room_name)

	IMPORTANT INFORMATION FROM THE RECEIVED SIP INVITE:
	FROM and CONTACT header of  contains the dialout number	
	TO header contains the room_name
	REASON header contains the reason for the invite i.e dial-out for dialout facility
	CALL-ID header contains te some random generatedn string

	
	LOGIC OF THE CODE:
	 
	=>This function ProcessSipEvent_cb waits for the sip events
	

	*/
	PString Reason="(No SIP Reason)";
	PTRACE(1, trace_section << "CDOT-MCU: Trying to get Reason and current value is : " << Reason);
	if(sip->sip_reason!=NULL)
	Reason=sip->sip_reason->re_text;
	PTRACE(1, trace_section << "CDOT-MCU: Printing Reason received : " << Reason);
	...
	if(Reason == "\"dial-out\"")
	{
		...
		PTRACE(1, trace_section << "CDOT-MCU: Case of dial-out ") ;
		//Softswitch ip
		PString softswitch_server_ip=OpenMCU::Current().GetSoftswitchServerIP();	
		//Extracting username for (To field) from the Recieved invite e.g 1001@192.168.1.198   ,here 1001 username for dialout
		sip_addr_t *Sip_from;
        	Sip_from = sip->sip_from;
        	PString To_username_for_dialout = Sip_from->a_url->url_user;
	

		//Extracting Call ID
		PString CallId="";
		CallId = sip->sip_call_id->i_id;
	
		//Extracting Display name //No use for us
		PString DisplayName="";	
		if(Sip_from->a_display && PString(Sip_from->a_display) != "")
	    	DisplayName = Sip_from->a_display; 	
	

		//extracting TO from received INVITE to create FROM(only username i.e username@host) for dialout(used in sip_make_call func)
		
		PString from_for_dialout="";
		from_for_dialout = sip->sip_to->a_url->url_user;


      		PString to_for_dialout =  "sip:"+To_username_for_dialout+"@"+softswitch_server_ip+":5060";
      		//PString call_id = "1543320159";
      		PString call_id = CallId+"V";
  			
		PTRACE(1, trace_section << "CDOT-MCU: Details for the dialout ");
		PTRACE(1, trace_section << "CDOT-MCU: Room name is  : " << from_for_dialout);
		PTRACE(1, trace_section << "CDOT-MCU: Printing TO created for dialout: " << to_for_dialout);
		PTRACE(1, trace_section << "CDOT-MCU: Printing Softswitch IP : " << softswitch_server_ip);
		PTRACE(1, trace_section << "CDOT-MCU: Printing  Callid received  :" << CallId <<"Sending Invite with Callid" << call_id);
		PTRACE(1, trace_section << "CDOT-MCU: Trying to dial-out to  %s : " << from_for_dialout);
		SipMakeCall(from_for_dialout,to_for_dialout,call_id);
	}

	//CDOT-MCU changes ENDS
PTRACE(1, trace_section << "CDOT-MCU: In ProcessSipEvent_cb()function : about to calling SipReqReply on METHOD=INVITE");
...
{
	PTRACE(1, trace_section << "vish: In HasListener() function");
...
{
	PTRACE(1, trace_section << "vish: In RemoveListeners() function");
  PWaitAndSignal m(sipMutex);
...
void MCUSipEndPoint::StartListeners()
{
	...
	PTRACE(1, trace_section << "vish: In StartListeners() function");
  PWaitAndSignal m(sipMutex);
...
{
	PTRACE(1, trace_section << "vish: In ProcessSipQueue() function");
  for(;;)
  {
	...
	PTRACE(1, trace_section << "vish: In ProcessSipQueue() function : Calling SipMakeCall() on REQUEST=INVITE");
...
void MCUSipEndPoint::MainLoop()
{
  while(1)
  {
	...
	PTRACE(1, trace_section << "vish: In MainLoop(): init_config test case : ");
      init_config = 0;
	PTRACE(1, trace_section << "vish: In MainLoop(): init_config test case : about to calling StartListeners()");
      StartListeners(); //vish : listen when invite comes
...
//	PTRACE(1, trace_section << "vish: In MainLoop(): Calling ProcessSipQueue ");
    ProcessSipQueue();
//	PTRACE(1, trace_section << "vish: In MainLoop(): Calling ProcessProxyAccount ");
    ProcessProxyAccount();
    su_root_sleep(root,500);
...
{
  PWaitAndSignal m(sipMutex);
	PTRACE(1, trace_section << "vish: In Terminating(): Calling ProcessSipQueue on Termination");
  ProcessSipQueue();
  ClearProxyAccounts();
  ClearSipCaps(BaseSipCaps);
  ClearStunList();
...
PTRACE(1, trace_section << "vish: :executing Main() " );  

	if(agent != NULL)
  {
PTRACE(1, trace_section << "vish: : In Main() : about to enter MainLoop()" );  
    MainLoop();
