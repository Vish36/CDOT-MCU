...
<p>\
CDOT-MCUサービスは動作しており、接続を受け付けています\
</p>\
<p>\
複数の端末から接続し開始された会議は、非管理モードで動作します。すべてのメンバーが同じ画像で表示されます。非管理モードでCDOT-MCUは自動的に「layouts.conf」ファイルで設定されたレイアウトを会議メンバーの数に応じて変更します。メンバーが１００以上接続された場合、表示は行いません。<br />\
